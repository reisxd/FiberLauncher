    ...
    Array.from(document.getElementsByClassName('settingsAuthAccountLogOut')).map((val) => {
        val.onclick = (e) => {
            ...
            if(Object.keys(ConfigManager.getAuthAccounts()).length === 1){
                isLastAccount = true
                setOverlayContent(
                    'Uyarı<br>Bu senin son hesabın',
                    'Başlatıcıyı kullanmak için en az bir hesapla oturum açmış olmanız gerekir. Daha sonra tekrar giriş yapmanız gerekecek. <br> <br> Çıkış yapmak istediğinizden emin misiniz?',
                    'Eminim',
                    'İptal'
                )
                setOverlayHandler(() => {
...
function refreshAuthAccountSelected(uuid){
    Array.from(document.getElementsByClassName('settingsAuthAccount')).map((val) => {
        ...
        if(uuid === val.getAttribute('uuid')){
            ...
            selBtn.innerHTML = 'Seçilmiş Hesap &#10004;'
        } else {
            if(selBtn.hasAttribute('selected')){
                selBtn.removeAttribute('selected')
            }
            selBtn.innerHTML = 'Hesabı Seç'
        }
    })
}
            ...
            <div class="settingsAuthAccountRight">
                <div class="settingsAuthAccountDetails">
                    <div class="settingsAuthAccountDetailPane">
                        <div class="settingsAuthAccountDetailTitle">Kullanıcı Adı</div>
                ...
                <div class="settingsAuthAccountActions">
                    <button class="settingsAuthAccountSelect" ${selectedUUID === acc.uuid ? 'selected>Seçilmiş Hesap &#10004;' : '>Hesap Seç'}</button>
                    <div class="settingsAuthAccountWrapper">
                        <button class="settingsAuthAccountLogOut">Çıkış Yap</button>
                    </div>
                </div>
            </div>
        </div>`
                        ...
                        <div class="settingsModMainWrapper">
                            ...
                            <div class="settingsModDetails">
                                ...
                                <div class="settingsDropinRemoveWrapper">
                                    <button class="settingsDropinRemoveButton" remmod="${dropin.fullName}">Kaldır</button>
                                </div>
                            </div>
                        </div>
                        <label class="toggleSwitch">
    ...
    Array.from(sEls).map((v, index, arr) => {
        v.onclick = () => {
            ...
            if(res){
                ...
                setOverlayContent(
                    `Silinemedi <br> Bırakma Modu ${fullName}`,
                    'Dosyanın kullanımda olmadığından emin olun ve tekrar deneyin.',
                    'Tamam'
                )
        ...
        if(dropinUI != null){
            ...
            if(DropinModUtil.isDropinModEnabled(dropin.fullName) != dropinUIEnabled){
                DropinModUtil.toggleDropinMod(CACHE_SETTINGS_MODS_DIR, dropin.fullName, dropinUIEnabled).catch(err => {
                    if(!isOverlayVisible()){
                        setOverlayContent(
                            'Modlarda <br> Bir veya Daha Fazla Bırakma Değiştirilemedi',
                            err.message,
                            'Tamam'
                        )
                        setOverlayHandler(null)
        ...
        <div class="serverListingDetails">
            ...
            <div class="serverListingInfo">
                ...
                ${serv.isMainServer() ? `<div class="serverListingStarWrapper">
                    ...
                    <span class="serverListingStarTooltip">Ana Sunucu</span>
                </div>` : ''}
            </div>
        </div>
    `
    ...
    jg._validateJavaBinary(execPath).then(v => {
        if(v.valid){
            ...
            if(v.version.major < 9) {
                settingsJavaExecDetails.innerHTML = `Seçildi: Java ${v.version.major} Güncelleme Sürümü ${v.version.update} (x${v.arch})${vendor}`
            } else {
                settingsJavaExecDetails.innerHTML = `Seçildi: Java ${v.version.major}.${v.version.minor}.${v.version.revision} (x${v.arch})${vendor}`
            }
        } else {
...
function populateVersionInformation(version, valueElement, titleElement, checkElement){
    ...
    if(isPrerelease(version)){
        titleElement.innerHTML = 'Ön sürüm'
        titleElement.style.color = '#ff886d'
        checkElement.style.background = '#ff886d'
    } else {
        titleElement.innerHTML = 'Kararlı Sürüm'
...
function populateSettingsUpdateInformation(data){
    if(data != null){
        ...
        if(process.platform === 'darwin'){
            settingsUpdateButtonStatus('GitHubdan İndir<span style="font-size: 10px;color: gray;text-shadow: none !important;">Başlatıcıyı kapatın ve güncellemek için dmgyi çalıştırın.</span>', false, () => {
                shell.openExternal(data.darwindownload)
            })
        } else {
            settingsUpdateButtonStatus('İndiriliyor..', true)
        }
    } else {
        settingsUpdateTitle.innerHTML = 'En Son Sürümü Çalıştırıyorsunuz'
        settingsUpdateChangelogCont.style.display = 'none'
        populateVersionInformation(remote.app.getVersion(), settingsUpdateVersionValue, settingsUpdateVersionTitle, settingsUpdateVersionCheck)
        settingsUpdateButtonStatus('Güncellemeleri kontrol et', false, () => {
            if(!isDev){
                ipcRenderer.send('autoUpdateAction', 'checkForUpdate')
                settingsUpdateButtonStatus('Güncellemeler Kontrol Ediliyor..', true)
            }
        })
    }
}
